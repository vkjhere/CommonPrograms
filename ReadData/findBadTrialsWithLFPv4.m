...
function [allBadTrials,badTrials] = findBadTrialsWithLFPv4(monkeyName,expDate,protocolName,folderSourceString,gridType, ...
    ...
    transientIgnorePeriod,transientThreshold)

if ~exist('checkTheseElectrodes','var');     checkTheseElectrodes = [33 12 80 63 44];   end
if ~exist('processAllElectrodes','var');     processAllElectrodes = 1;                  end
...
if ~exist('checkPeriod','var');              checkPeriod = [-0.5 0.5];                  end
if ~exist('rejectTolerance','var');          rejectTolerance = 1;                       end
if ~exist('showElectrodes','var');           showElectrodes = [];                       end
if ~exist('transientIgnorePeriod','var');    transientIgnorePeriod = [0 0.3];           end
if ~exist('transientThreshold','var');       transientThreshold = 1.5e5;                end
...
for i=1:numElectrodes
    ...
    %*********************************************************************
    % [Siddhesh] EXTRA CHECK TO DETECT TRANSIENTS ACROSS ELECTRODES
    %---------------------------------------------------------------------
    % [Siddhesh] - check for trials in which there are transients crossing
    % a transient threshold, across time, except the ignore period
    transientIgnorePeriodIndices = intersect(find(timeVals>=transientIgnorePeriod(1)),find(timeVals<=transientIgnorePeriod(2)));
    if transientIgnorePeriodIndices(1) >= find(checkPeriodIndices,1,'first') && transientIgnorePeriodIndices(1) <= find(checkPeriodIndices,1,'last')
        transientIgnorePeriodIndices = transientIgnorePeriodIndices - find(checkPeriodIndices,1) + 1;
    else
        error('Transient ignore period cannot be outside check period.');
    end
    dVBydt = diff(analogData,1,2)/(timeVals(2)-timeVals(1));
    clear transientTimes
    for j = 1:numTrials
        transientTimes{j} = union(find(dVBydt(j,:) > transientThreshold),find(dVBydt(j,:) < -transientThreshold)); %#ok<*AGROW>
        transientTimes{j} = setdiff(transientTimes{j},transientIgnorePeriodIndices);
    end
    badTrialsTransients = find(cellfun(@(x)~isempty(x),transientTimes));
    allBadTrials{electrodeNum} = union(allBadTrials{electrodeNum},badTrialsTransients);
end
...
if saveDataFlag
    ...
    save(fullfile(folderSegment,'badTrials.mat'),'badTrials','checkTheseElectrodes','threshold','maxLimit','minLimit','checkPeriod','allBadTrials','nameElec','rejectTolerance','badElecs','badTrialsMarginalStats', ...
        'transientThreshold','transientIgnorePeriod');
else
